...
#ExerciseController is in 'exercise_controller.py'
from exercise_controller import ExerciseController  

#TESTING: create a dummy node to call exercise controller
class DummyNode:
    def __init__(self):
        # Initialize the ROS node
        rospy.init_node('dummy_node', anonymous=True)
        
        # parameters for initializing ExerciseController
        replay = False
        log_filename = 'exercise_log.txt' #just a file to record?
        style = 2  # Example: neutral style
        resting_hr = 60
        max_hr = 180
        user_id = 'test user'
        
        # Initialize the ExerciseController
        self.exercise_controller = ExerciseController(replay, log_filename, style, resting_hr, max_hr, user_id)

        # Subscribe to joint angles and heart rate topics
        self.pose_sub = rospy.Subscriber("/joint_angles", Float64MultiArray, self.pose_callback)
        self.heart_rate_sub = rospy.Subscriber("/heart_rate", Int32, self.heart_rate_callback)
        
    def pose_callback(self, msg):
        # Call the `pose_callback` method from ExerciseController
        self.exercise_controller.pose_callback(msg)
        
    def heart_rate_callback(self, msg):
        # Call the `heart_rate_callback` method from ExerciseController
        self.exercise_controller.heart_rate_callback(msg)
...
if __name__ == '__main__':
    ...
    #TESTING:
    #create new dummy node to call exercise controller
    # Initialize and run the dummy node
